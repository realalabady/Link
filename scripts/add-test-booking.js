...
async function createTestBooking() {
  ...
  console.log(`Found client: ${client.displayName || client.name || client.email} (${client.id})`);
  ...
  console.log(`Found provider: ${provider.displayName || provider.name || provider.email} (${provider.id})`);
  ...
  console.log(`Using service: ${service.title || service.name} (${service.id})`);
  ...
  const bookingData = {
    ...
    clientName: client.displayName || client.name || client.email?.split("@")[0] || "Test Client",
  ...
  console.log(`Provider: ${provider.displayName || provider.name || provider.email}`);
